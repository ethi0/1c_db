...
RUN apt-get -qq update \
  && apt-get -qq install --yes --no-install-recommends ca-certificates wget locales-all locales\
  && apt-get clean \
...
RUN update-locale LANG=uk_UA.UTF-8

RUN localedef --inputfile uk_UA --force --charmap UTF-8 --alias-file /usr/share/locale/locale.alias uk_UA.UTF-8
